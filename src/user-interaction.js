class UserInput extends HTMLElement {
    ...
    attributeChangedCallback (name, oldValue, newValue) {
        if (name === 'state-response') {
            if(newValue == "true"){
                this.shadow.querySelector(".stop-button").classList.remove("hide")
                this.shadow.querySelector(".send-button").classList.add("hide")
            }else{
                this.shadow.querySelector(".stop-button").classList.add("hide")
                this.shadow.querySelector(".send-button").classList.remove("hide")
            }
        }
    }
    ...
    render() {
      ...
      /*html*/`
            ...
            <section class="message-input">
                <form>
                    ...
                    <div class="form-element">
                        <textarea placeholder="Message ChatGPT..." class="input-text" data-minlenght="1"></textarea>
                    </div>
        ...
        const disabledSendButton = this.shadow.querySelector(".send-button button")
        const stopButton = this.shadow.querySelector(".stop-button")
        const disabledStopButton = this.shadow.querySelector(".stop-button button")

        input.addEventListener("input", () => {

            if (input.value.length > 0) {
                sendButton.classList.add("active");
                disabledSendButton.disabled = false;
                disabledStopButton.disabled = true;
            }

            if (input.value.length <= 0) {
                sendButton.classList.remove("active");
                disabledSendButton.disabled = true;
                disabledStopButton.disabled = true;
            }
        ...
        sendButton.addEventListener("click", (event) =>{            
            ...
            this.setAttribute('state-response', true)
        })

        stopButton.addEventListener("click", (event) =>{            
            event.preventDefault();
            this.setAttribute('state-response', false)
        })
    }
...
customElements.define('input-component', UserInput);